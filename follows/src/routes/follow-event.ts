...
router.post(
    '/api/follows/goer-follows/event', 
    requireAuth,
    [
    ...
    try {
        await EventFollowTransactions.followEvent(currentGoerEventFollows, eventFollowers, currentUserObjectId, eventObjectId);
    } catch (err) {
        console.log(err);
...
export { router as followEventRouter };